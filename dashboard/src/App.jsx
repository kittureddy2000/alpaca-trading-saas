...
import { getPortfolio, getPositions, getMarket, getWatchlist, getTrades, getIndicators, getCurrentUser, getLoginUrl, getLogoutUrl, register, login, googleTokenLogin, getOptionChain, getCollarStrategy, addToWatchlist, removeFromWatchlist, setTokens, clearTokens, getAccessToken, getAlpacaStatus, connectAlpaca, disconnectAlpaca, getSettings, updateSettings, getSubscription, createCheckoutSession, createPortalSession, getAlpacaOAuthUrl, connectAlpacaOAuth } from './api';
...
// ============================================================
// Alpaca Connect Page Component
// ============================================================
function AlpacaConnectPage({ subscription }) {
  ...
  const fetchStatus = async () => {
    try {
      const data = await getAlpacaStatus();
      setStatus(data);
    } catch (e) {
  ...
  const handleConnect = async (e) => {
    ...
    try {
      await connectAlpaca(formData.apiKey, formData.secretKey, formData.paper);
      setSuccess('Alpaca account connected successfully!');
      setFormData({ apiKey: '', secretKey: '', paper: true });
      fetchStatus();
    } catch (e) {
      setError(e.response?.data?.error || 'Failed to connect Alpaca account');
    } finally {
      setConnecting(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    if (!window.confirm('Are you sure you want to disconnect your Alpaca account?')) return;
  ...
  return (
    <div className="settings-page">
      <div className="settings-header">
        <h1>Alpaca Connection</h1>
        <p>Connect your Alpaca account to enable trading</p>
      </div>
      ...
      {status?.connected ? (
        <div className="settings-card">
          <div className="connection-status connected">
            ...
            <span>Connected to Alpaca</span>
          </div>
          <div className="connection-details">
            <div className="detail-row">
              <span className="label">Account ID:</span>
              <span className="value">{status.account_id}</span>
            </div>
            <div className="detail-row">
              <span className="label">Mode:</span>
              <span className={`value ${status.is_paper ? 'paper' : 'live'}`}>
            ...
            <div className="detail-row">
              ...
              <span className="value">{new Date(status.connected_at).toLocaleString()}</span>
            </div>
          </div>
  ...
  return (
    <div className="billing-page">
      ...
      {subscription?.tier !== 'free' && (
        <div className="current-plan-banner">
          <div className="plan-info">
            ...
            <span className="plan-name">{subscription?.tier?.toUpperCase()}</span>
      ...
      <div className="pricing-grid">
        {tiers.map((tier) => (
          <div key={tier.id} className={`pricing-card ${tier.popular ? 'popular' : ''} ${subscription?.tier === tier.id ? 'current' : ''}`}>
            {tier.popular && <div className="popular-badge">Most Popular</div>}
            {subscription?.tier === tier.id && <div className="current-badge">Current Plan</div>}
            ...
            {subscription?.tier === tier.id ? (
              ...
              >
                {processingTier === tier.id ? 'Processing...' : `Upgrade to ${tier.name}`}
              </button>
            )}
          </div>
        ))}
...
function App() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [portfolioData, marketData, watchlistData, tradesData, subscriptionData, alpacaStatusData, settingsData] = await Promise.all([
        getPortfolio(),
        getMarket(),
        getWatchlist(),
        getTrades(),
      ...
      setAlpacaStatus(alpacaStatusData);
  ...
  return (
    <div className="dashboard">
            ...
            {profileOpen && (
              <div className="profile-dropdown">
                ...
                <div className="profile-section">
                  <div className="profile-section-title">Trading Settings</div>
                  <div className="config-grid">
                    <div className="config-item"><span className="label">Strategy</span><span className="value">{userSettings?.strategy || 'balanced'}</span></div>
                    <div className="config-item"><span className="label">Max Position %</span><span className="value">{((userSettings?.max_position_pct || 0.10) * 100).toFixed(0)}%</span></div>
                    <div className="config-item"><span className="label">Max Daily Loss</span><span className="value">{((userSettings?.max_daily_loss_pct || 0.03) * 100).toFixed(0)}%</span></div>
                    <div className="config-item"><span className="label">Min Confidence</span><span className="value">{((userSettings?.min_confidence || 0.70) * 100).toFixed(0)}%</span></div>
                    <div className="config-item"><span className="label">Stop Loss</span><span className="value">{((userSettings?.stop_loss_pct || 0.05) * 100).toFixed(0)}%</span></div>
                    <div className="config-item"><span className="label">Take Profit</span><span className="value">{((userSettings?.take_profit_pct || 0.10) * 100).toFixed(0)}%</span></div>
                  </div>
                </div>
      ...
      <nav className="nav-tabs">
        ...
        <button
          className={`nav-tab ${currentPage === 'billing' ? 'active' : ''}`}
